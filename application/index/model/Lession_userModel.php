...
class Lession_userModel extends Model
{
    ...
    public function updateOneRecord($condition,$lessionID,$userID){
        try{
            $res = $this->db->where('lessionID',$lessionID)->where('userID',$userID)->update($condition);
        }catch(\Exception $e){
            $res =$e->getMessage();
        }
        if($res === 1){
